...
| Phase E rating-layer validation | Python | **V1 done** — `24_rating_layer_validation` (§23): all 11 family magnitudes analytically 1.0× the historical anchor; paired ON-vs-OFF league check (100 pairs) shows the centred layer costs ~0.7 pts/team-game (was ~1.2 before the wide residual window) and still narrows margin variance where §12 predicts a widen — a §13.6 joint-calibration item (same "per-play right, drive-model wrong" pattern as the §22 points gap) |
