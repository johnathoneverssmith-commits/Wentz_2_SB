...
| Portable HGB export | Python + TS | **done + wired** — `27_export_portable` + `lib_py/hgb_portable`: the 7 HGB resolvers → `artifacts/models/portable/*.json`, matches sklearn to <1e-6. `engine/loaders` loads these (joblib only for the 6 logistic resolvers) → §22 unchanged, ~4× faster. `src/engine/hgb-portable.ts` is the verified TS port |
