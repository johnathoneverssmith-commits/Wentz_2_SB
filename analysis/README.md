...
| Phase E full Monte-Carlo validation | Python | **V1 done** — `engine/` + `23_full_sim_validation` (§22, **16/20 within 10%**). V1.6 points-gap work (`docs/v16_points_gap_plan.md`) took points/team-game **−11.8% → −2.0%**: `CLOCK_SCALE 0.958` (M24 snap gaps ran ~1 s/play long), the `rz_yac` table (goal-line completions caught short score at the empirical reach rate), end-of-half clock management (2-min-drill timeouts / spikes / kick unit — a play-by-play-mode prerequisite), two punt field-position bugs, RZ `air_yards` split by goal-line distance, `PICK_SIX_RATE`/`SCOOP_SIX_RATE` ~5× low, punt-return-TD drive labeling. 4 remaining §22 fails all expected (`points_sd` = rating layer OFF; 3 deliberate penalty-volume metrics). |
| Phase E rating-layer validation | Python | **V1 done** — `24_rating_layer_validation` (§23): all 11 family magnitudes analytically 1.0× the historical anchor. Paired ON-vs-OFF (deterministic after the `crc32` seed fix): score-margin sd **widens** (§12 direction), points delta **≈ −0.8 ± 0.4 /team-game** (z≈1.9, ~−4%, diffuse) — a §13.6 *watch* item, not a coefficient change. The old "margin narrows / −1.2 pts" read was an unpinned-`hash()` artifact. |
| Phase E §13.6 joint calibration | Python | **harness done** — `26_joint_calibration` (`converge` / `diagnose` / `curvature_probe`): measures the emergent points/margin targets the per-play anchors don't constrain. Verdict: layer is emergently sound; no βᵢ change. `artifacts/validation/joint_calibration_probe.json`. |
