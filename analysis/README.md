...
| Phase E full Monte-Carlo validation | Python | **V1 done** — `engine/` + `23_full_sim_validation` (§22, **16/20 within 10%**). V1.6 points-gap work (`docs/v16_points_gap_plan.md`) took points/team-game **−11.8% → −4.9%**: fixed two punt bugs (field position), split the RZ `air_yards` table by exact goal-line distance, `CLOCK_SCALE 0.958` (M24 snap gaps ran ~1 s/play long), `PICK_SIX_RATE`/`SCOOP_SIX_RATE` were ~5× low. 4 remaining §22 fails all expected (`points_sd` = rating layer OFF; 3 deliberate penalty-volume metrics). |
